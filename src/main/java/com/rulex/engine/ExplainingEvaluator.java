...
public class ExplainingEvaluator extends RuleBaseVisitor<TraceNode> {
    ...
    private TraceNode safeVisit(ParserRuleContext ctx) {
        try {
            return visit(ctx);
        } catch (Exception e) {
    ...
    private TraceNode tryLeaf(ParserRuleContext ctx, String type, Supplier<TraceNode> compute) {
        try {
            return compute.get();
        } catch (Exception e) {
    ...
    public TraceNode visitOrExpr(RuleParser.OrExprContext ctx) {
        TraceNode left = safeVisit(ctx.expr(0));
        TraceNode right = safeVisit(ctx.expr(1));
    ...
    public TraceNode visitNotExpr(RuleParser.NotExprContext ctx) {
        TraceNode child = safeVisit(ctx.expr());
    ...
    public TraceNode visitIsNotNullPred(RuleParser.IsNotNullPredContext ctx) {
        return tryLeaf(ctx, "IS_NOT_NULL", () -> {
            RuleValue val = eval.visit(ctx.arith());
